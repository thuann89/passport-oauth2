...
const getOAuthAccessToken = function (code, params, callback) {
  var opts = {
    auth: {
      user: params['client_id'],
      pass: params['client_secret']
    },
  ...
  rp.post(params['token_url'], opts)
    .then(function (resp) {
      ...
      callback(null, resp.access_token, null, result);
    })
    .catch(err => {
      // console.log(err);
      callback(err)
...
OAuth2Strategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.code) {
    function loaded(err, ok, state) {
      ...
      params['token_url'] = self._oauth2._accessTokenUrl;
      params.grant_type = 'authorization_code';
      if (callbackURL) { params.redirectUri = callbackURL; }
    ...
    if (state) {
      ...
      parsed.query['clientId'] = this._oauth2._clientId;
      ...
      function stored(err, state) {
        ...
        utils.merge(parsed.query, params);
        parsed.query['clientId'] = self._oauth2._clientId;
        delete parsed.search;
        var location = url.format(parsed);
